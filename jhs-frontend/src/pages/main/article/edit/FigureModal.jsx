...
import { addArticleFigure, getDraftArticle, updateArticleFigure } from 'store/main/articles/actions';
...
const FigureModel = ({ figureEditId, articleId, editor, figureData, setFigureEditId, figureLength = 0 }) => {
    ...
    const updateFigureHandle = async (formData) => {
        ...
        if (figureData) {
            axios.put(import.meta.env.VITE_REACT_APP_API_URL + `article/${articleId}/updateFigure`, appendForm, {
                headers: {
                ...
                .then(response => {
                    ...
                    dispatch(getDraftArticle({ body: {}, options: { id: articleId, btnLoader: true, __module: 'article', } }));    //payload.options.id
                })
                .catch(error => {
                    console.error(error);
                });
        } else {
            axios.put(import.meta.env.VITE_REACT_APP_API_URL + `article/${articleId}/addFigure`, appendForm, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
                    ...mainConfig.data().defaultHeaders,
