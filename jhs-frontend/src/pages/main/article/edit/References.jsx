...
import { updateReferenceSorting, addReference, removeReference, generateReference, getSpecificReference } from '../../../../store/main/reference/actions';
...
import UpdateReferenceModel from '../operations/UpdateReferenceModel';
...
const Reference = () => {
    ...
    useEffect(() => {
        reset();
        const arr = [];
        if (articleInfo?.article_data_id?.reference?.length) {
            articleInfo.article_data_id.reference.map((item) => {
    ...
    const updateReferenceSortingHandle = () => {
        ...
        navigate(`/main/article/${articleId}/edit/reviewers`);
    }

    const updateReferenceHandler = (referenceId) => {
        dispatch(
            getSpecificReference({
                options: { id: articleId, referenceId: referenceId, btnLoader: true, __module: 'article', showToast: false },
            }));
