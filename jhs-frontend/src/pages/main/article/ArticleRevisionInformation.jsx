...
import { getArticleRevision, getArticleRevisionReferencesTextList } from 'store/main/articlesRevision/actions';
import ArticleActions from './operations/ArticleActions';

const ArticleRevisionInformation = () => {
    const dispatch = useDispatch();
    let { articleId } = useParams();
    const articleInfo = useSelector((state) => state.article.single);
    const articleRevisionInfo = useSelector((state) => state.articleRevision.single);
    const singleOldReferenceTextList = useSelector((state) => state.articleRevision.singleOldReferenceTextList);
    const singleNewReferenceTextList = useSelector((state) => state.articleRevision.singleNewReferenceTextList);
    ...
    useEffect(() => {
        ...
        dispatch(getArticleRevisionReferencesTextList({ body: {}, options: { id: articleId, btnLoader: true, __module: 'articleRevision', } })); 
    ...
    const htmlContentParser = (htmlString, item) => {
        ...
        let revisionTableCount = calcPreviousTableCount(item);
        let revisionFigureCount = calcPreviousFigureCount(item);
            ...
            replace: (node, index) => {
                if (
                    ...
                    if(node.attribs["type"] !== "revision"){
                        ...
                        tableData = articleRevisionInfo.articleRevision_data_id.table_list.find(({ _id }) => _id === dataId);
                        if (tableData) {
                            revisionTableCount++
                            return <ArticleTableView tableData={tableData} tableNo={revisionTableCount} />
                        }
                    }
                }
                ...
                if (
                    ...
                    figureData = articleRevisionInfo.articleRevision_data_id.figures_list.find(({ _id }) => _id === dataId);
                    if (figureData) {
                        revisionFigureCount++
                        return <ArticleFigureView figureData={figureData} figureNo={revisionFigureCount} />
                    }
                }
                return undefined
            }
                    ...
                    <div className="col-lg-8 pb-2 pb-md-0 mb-4 mb-md-0">
                        <div className="card shadow border-1 rounded-3 mb-3" >
                            <div className="card-body">
                                ...
                                >
                                    {articleInfo?.abstract ? htmlContentParser(calculateChanges(articleInfo?.abstract, articleRevisionInfo?.abstract ?? ''), 'abstract') : ''}
                                </div>
                                    ...
                                    <h2 className="h6 py-1 mb-0 me-4">Keywords:</h2>
                                    {
                                        articleInfo?.keywords?.map((keyword, index) => {
                                            return <a key={"keywords" + index} className="nav-link fs-sm py-1 px-0 me-3" href="#">
                                                <span className="text-primary">#</span>{htmlContentParser(calculateChanges(keyword, articleRevisionInfo?.keywords[index] ?? ''))}
                                            </a>
                                        })
                                    }
                                </div>
                                                        ...
                                                        return (
                                                            <div key={`article_element_type_${index}`} className={`tab-pane fade show ${index == 0 ? 'active' : ''}`} id={`article${item}`} role="tabpanel">
                                                                ...
                                                                <div style={{ textAlign: "justify", textJustify: "inter-word" }} >
                                                                    {htmlContentParser(calculateChanges(articleInfo?.article_data_id[`${item}`] ?? '', articleRevisionInfo?.articleRevision_data_id[`${item}`] ?? ''), item)}
                                                                </div>
                                                            </div>
                                                        )
                                                    })
...
export default ArticleRevisionInformation;