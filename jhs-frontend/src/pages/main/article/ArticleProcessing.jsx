...
import BlogWithOverlayHighlight from "./BlogWithOverlayHighlight";
...
const ArticleProcessing = () => {
  ...
  const singleReferencesTextList = useSelector(
    (state) => state.article.singleReferenceTextList
  );
  const articleTypes = useSelector((state) => state.articleType.list);
  const selectedArticleType = useSelector(
    (state) => state.articleType.selected
  );
...
//   }, [comments]);
  ...
  useEffect(() => {
    dispatch(
      getArticle({
        body: {},
        options: { id: articleId, btnLoader: true, __module: "article" },
      })
    );
    dispatch(
      getArticleReferencesTextList({
    ...
    dispatch(
      getAllArticleType({ body: {}, options: { __module: "articleType" } })
    );
  }, [dispatch, articleId]);
  ...
  useEffect(() => {
    ...
    if (socketSession.status) {
      ...
      dispatch(send(socketSession, "SET_MY_ARTICLE_COMMENTS", { articleId }));
  ...
  const calcPreviousTableCount = (currentArticleType) => {
    ...
    for (let i = pageIndex - 1; i >= 0; i--) {
      let data =
        articleInfo?.article_data_id?.[selectedArticleType?.elements[i]];
      if (data) {
    ...
    const parsedHtml = parse(data, {
      replace: (node, index) => {
        if (
          ...
          let tableData = articleInfo.article_data_id.table_list.find(
            ({ _id }) => _id === dataId
          );
          if (tableData) {
            return (
        ...
        if (
          ...
          figureCount++;
          const dataId = node.attribs["data-id"];
          let figureData = articleInfo.article_data_id.figures_list.find(
            ({ _id }) => _id === dataId
          );
          if (figureData) {
            return (
  ...
  function highlight(
    ...
    if (alreadyHighlight == null && articleInfo) {
          ...
          innerHTML =
            ...
            `<span 
                                                ...
                                                style='background-color: ${
                                                  getHighlightColorById(
                                                    commentUserInfo?._id
                                                  )?.background
                                                }; border: 2px solid; padding: 0 2px; border-color: ${
              getHighlightColorById(commentUserInfo?._id)?.borderColor
            };'>
                                                    ${secondPart}
                                                    <div class="hightlight-popover">
                                                        <div>
                                                            <img style="width: 40px; height: 40px;" src="/assets/img/avatar/user.png" alt="${
    ...
    <div className="mx-4 px-4">
          ...
          <div className="row mt-sm-n1 mb-0">
            <div className="col-10">
              ...
              <div className="d-flex flex-wrap align-items-center mt-n2">
                ...
                >
                  <ArticleStatusName
                    taskState={articleInfo?.articleStatus?.slug}
                    article={articleInfo}
                  />
                </a>
              ...
              <div className="d-flex flex-row-reverse mt-sm-n1 mb-0 mb-lg-1 mb-xl-3">
                ...
                <div className="dropdown dropdown-menu dropdown-menu-end my-1">
                  ...
                  <button
                    ...
                    onClick={() => deleteArticleHandler(articleInfo?._id)}
                  >
      ...
      <section className="mt-2 mt-md-0 pb-5 mb-md-2 mb-lg-3 mb-xl-4 mb-xxl-5">
        ...
        >
          ...
          <aside className="col-lg-2 mb-4" style={{ marginTop: "-115px", zIndex: 10, }}>
            <div
              ...
              <div className="position-lg-sticky top-0">
                ...
                >
                  ...
                  <div className="offcanvas-body ml-4">
                    ...
                    <li className="nav flex-column pb-1 mb-1">
                      ...
                      <ul className="nav flex-column border-start ps-3 ms-2 mb-2">
                        <li className="nav-item">
                          <a
                            className={`nav-link fs-sm fw-normal py-0 ps-1 pe-0 mb-1 ${
                              location.pathname ===
                    ...
                    <li className="nav flex-column pb-1 mb-1">
                      <span className="nav-link px-0 py-1">
                        <i className="ai-user fs-lg opacity-90 me-2"></i>Author
                      </span>
                      <ul className="nav flex-column border-start ps-3 ms-2 mb-2">
                        <li className="nav-item">
                          <a
                            className={`nav-link fs-sm fw-normal py-0 ps-1 pe-0 mb-1 ${
                              location.pathname ===
                              `/main/article/${articleId}/process#author`
                                ? "active"
                                : ""
                            } `}
                            href={"#author"}
                        ...
                        {selectedArticleType?.elements?.map((item, index) => {
                          return (
                            <li className="nav-item" key={"pageLink" + item}>
                              <a
                                className={`nav-link text-capitalize fs-sm fw-normal py-0 ps-1 pe-0 mb-1 ${
                                  location.pathname ===
                                  `/main/article/${articleId}/process#${item}`
                                    ? "active"
                                    : ""
                                }`}
                                href={`#${item}`}
                    ...
                    <li className="nav flex-column pb-1 mb-1">
                      <span className="nav-link px-0 py-1">
                        <i className="ai-table fs-lg opacity-90 me-2"></i>Table
                      </span>
                    </li>
                    <li className="nav flex-column pb-1 mb-1">
                      <span className="nav-link px-0 py-1">
                        <i className="ai-image fs-lg opacity-90 me-2"></i>
                        Figures
                      </span>
                    </li>
                    <li className="nav flex-column pb-1 mb-1">
                      <a
                        className={`nav-link px-0 py-1 ${
                          location.pathname ===
                          `/main/article/${articleId}/process#reviewers`
                            ? "active"
                            : ""
                        }`}
                        href={"#reviewers"}
                      >
                        <i className="ai-user-group fs-lg opacity-90 me-2"></i>
                        Reviewers
                      </a>
                    </li>
                    <li className="nav flex-column pb-1 mb-1">
                      <a
                        className={`nav-link px-0 py-1 ${
                          location.pathname ===
                          `/main/article/${articleId}/process#summary`
                            ? "active"
                            : ""
                        }`}
                        href={"#summary"}
                      >
                        <i className="ai-align-justify fs-lg opacity-90 me-2"></i>
                        Summary
                      </a>
                    </li>
                    <li className="nav flex-column pb-1 mb-1">
                      <a
                        className={`nav-link px-0 py-1 ${
                          location.pathname ===
                          `/main/article/${articleId}/process#submit`
                            ? "active"
                            : ""
                        }`}
                        href={"#submit"}
                      >
                        <i className="ai-checks fs-lg opacity-90 me-2"></i>
                        Submit
                      </a>
                    </li>
                  </div>
                  ...
                  <div className="card-body">
                    ...
                    <div className="d-flex flex-wrap pt-3 pt-md-3 pt-xl-3 mt-xl-n2 mb-4 ">
                      ...
                      {articleInfo?.keywords?.map((keyword, index) => {
                        return (
                          <a
                            key={`articleInfoKeyword_${index}`}
                            className="nav-link fs-sm py-1 px-0 me-3"
                            href="#"
                          >
                    ...
                    {selectedArticleType?.elements?.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          >
                            {htmlContentParser(
                              articleInfo?.article_data_id[`${item}`],
                              item
                            )}
                          </div>
                        </div>
          ...
          {/* Relevant article sidebar */}
          <aside className="col-lg-3 mb-2" style={{ marginTop: "-115px" }}>
            <div
            ...
            >
              ...
              <div className="card border-0 bg-transparent">
                <div className="card-body d-flex justify-content-center align-items-center border-dashed border-2 rounded-3 py-4 mb-3">
                  <a
                    className="h5 text-nav text-center text-transparent mb-0"
                    onClick={() => {
                  ...
                  {comments?.map((commentItem, index) => {
                    ...
                    return (
                      <div
                        ...
                        key={"commentItemInfoKey" + index}
                      >
                        <h3
                          className="accordion-header"
                          id={"commentItemInfo" + index}
                        >
                          <button
                          ...
                          >
                            ...
                            <div className="rounded-circle bg-size-cover bg-position-center flex-shrink-0"
                                style={{
                                  border: `3px solid ${getHighlightColorById(commentItem.addBy)?.borderColor}`, // Use dynamic border color from the function
                                  ...
                                  backgroundImage: `url(${
                                    commentUserInfo?.file
                                      ? `${import.meta.env.VITE_REACT_APP_URL}/public/uploads/profile/${commentUserInfo.file}`
                                      : '/assets/img/avatar/user.png'
                                  })`,
                                  backgroundSize: 'cover', // Ensure image covers the area
                                  backgroundPosition: 'center', // Center the image
                                  boxShadow: `0 8px 13px ${getHighlightColorById(commentItem.addBy)?.background}`, // Add a shadow based on border color
                                }}
                                alt="Comment author"
                              />
                              ...
                              <div className="ps-3">
                                <h6 className="mb-0">
                                  {commentUserInfo?.name ?? "System User"}
                                </h6>
                                  ...
                                  <div className="dropdown dropdown-menu dropdown-menu-end my-1">
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        //   commentItem._id,
                                        //   commentItem.text,
                                        //   commentItem.startOffset,
                                        //   commentItem.endOffset,
                                        //   commentItem.forArea,
                                        //   commentUserInfo
                                        // );
                                      }}
                                    >
                                      <i className="ai-edit-alt me-2"></i>{" "}
                                      Highlight Text
                                    </button>
                                    <button
                                      className="dropdown-item"
                                    ...
                                    >
                                      <i className="ai-dashboard me-2"></i> Show
                                      Comment Discussion
                                    </button>
