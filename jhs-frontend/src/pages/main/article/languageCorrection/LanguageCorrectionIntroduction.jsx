...
const LanguageCorrectionIntroduction = () => {
    ...
    const [editorData, setEditorData] = useState(articleInfo?.articleLanguageCorrection_data_id?.introduction ?? '');

    useEffect(() => {
        setEditorData(articleInfo?.articleLanguageCorrection_data_id?.introduction ?? '')
    }, [articleInfo?.articleLanguageCorrection_data_id?.introduction])
    ...
    const updateDraftArticleHandle = (formData) => {
        dispatch(
            updateArticleLanguageCorrection({
                body: { "introduction": editorData },
                options: { id: articleId, btnLoader: true, __module: 'articleLanguageCorrection', showToast: true },
            }));

        let pageIndex = selectedArticleType?.elements.indexOf("introduction");
                                        ...
                                        <div className="text-center text-lg-start ">
                                            ...
                                            <div className="pb-3">
                                                {articleInfo && <CKEditor
                                                    ...
                                                    config={{
                                                        plugins: [Essentials, Heading, Bold, Italic, Subscript, Superscript, Underline, CkLink, Paragraph],
                                                    ...
                                                    data={editorData}
                                                />}
                                                <div className="invalid-feedback">{errors.introduction?.message}</div>
                                            </div>
                                        </div>
                                    </div>
...
export default LanguageCorrectionIntroduction;