...
const LanguageCorrectionFigureModel = ({ figureEditId, articleId, editor, figureData, setFigureEditId, figureLength = 0 }) => {
    ...
    const updateFigureHandle = async (formData) => {
        ...
        if (figureData) {
            axios.put(import.meta.env.VITE_REACT_APP_API_URL + `articleLanguageCorrection/${articleId}/languageCorrection/uploadLanguageCorrectionFigure`, appendForm, {
                headers: {
                ...
                .then(response => {
                    ...
                    dispatch(getArticleLanguageCorrection({ body: {}, options: { id: articleId, btnLoader: true, __module: 'articleLanguageCorrection', } }));    //payload.options.id
                })
                .catch(error => {
                    console.error(error);
                });
        } else {
            axios.put(import.meta.env.VITE_REACT_APP_API_URL + `articleLanguageCorrection/${articleId}/languageCorrection/addLanguageCorrectionFigure`, appendForm, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
                    ...mainConfig.data().defaultHeaders,
...
export default LanguageCorrectionFigureModel;