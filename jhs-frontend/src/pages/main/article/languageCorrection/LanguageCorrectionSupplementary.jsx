...
const LanguageCorrectionSupplementary = () => {
    ...
    const [editorData, setEditorData] = useState(articleInfo?.articleLanguageCorrection_data_id?.supplementary ?? '');

    useEffect(() => {
        setEditorData(articleInfo?.articleLanguageCorrection_data_id?.supplementary ?? '')
    }, [articleInfo?.articleLanguageCorrection_data_id?.supplementary])
    ...
    const updateDraftArticleHandle = (formData) => {
        dispatch(
            updateArticleLanguageCorrection({
                body: { "supplementary": editorData },
                options: { id: articleId, btnLoader: true, __module: 'articleLanguageCorrection', showToast: true },
            }));

        let pageIndex = selectedArticleType?.elements.indexOf("supplementary");
                                        ...
                                        <div className="text-center text-lg-start ">
                                            ...
                                            <div className="pb-3">
                                                {articleInfo && <CKEditor
                                                    ...
                                                    data={editorData}
                                                />}
                                                <div className="invalid-feedback">{errors.supplementary?.message}</div>
                                            </div>
                                        </div>
                                    </div>
...
export default LanguageCorrectionSupplementary;