...
const LanguageCorrectionAcknowledgement = () => {
    ...
    const [editorData, setEditorData] = useState(articleInfo?.articleLanguageCorrection_data_id?.acknowledgement ?? '');

    useEffect(() => {
        setEditorData(articleInfo?.articleLanguageCorrection_data_id?.acknowledgement ?? '')
    }, [articleInfo?.articleLanguageCorrection_data_id?.acknowledgement])
    ...
    const updateDraftArticleHandle = (formData) => {
        dispatch(
            updateArticleLanguageCorrection({
                body: { "acknowledgement": editorData },
                options: { id: articleId, btnLoader: true, __module: 'articleLanguageCorrection', showToast: true },
            }));

        let pageIndex = selectedArticleType?.elements.indexOf("acknowledgement");
                                        ...
                                        <div className="text-center text-lg-start ">
                                            ...
                                            <div className="pb-3">
                                                {articleInfo && <CKEditor
                                                    ...
                                                    data={editorData}
                                                />}
                                                <div className="invalid-feedback">{errors.acknowledgement?.message}</div>
                                            </div>
                                        </div>
                                    </div>
...
export default LanguageCorrectionAcknowledgement;