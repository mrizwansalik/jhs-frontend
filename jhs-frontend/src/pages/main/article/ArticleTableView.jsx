...
const ArticleTableView = ({ id, data, setTableEditId, editor, articleId }) => {
  ...
  const handleDelete = () => {
    const event = new CustomEvent("journalArticleDelete", { detail: { tableId: id } });
  ...
  const tableSort = () => {
    editor.model.change(writer => {
      ...
      for (const value of range.getWalker()) {
        ...
        if (value.type === "elementStart" && value.item.is("element", "tablePreview")) {
          indexArr.push(value.item.index);
        }
      }
      for (const value of range.getWalker()) {

        if (value.item.is("element", "tablePreview") && value?.item?._attrs?.get('id') === id) {

          setMetaDetails((prev) => ({
            // index: JSON.stringify({index}),
            index: indexArr?.indexOf(value.item.index) + 1,
