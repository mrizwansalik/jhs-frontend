...
class EmployeePublishedArticleHelper extends PublishedArticleHelper {
    ...
    async getArticleDetail(articleId, userId) {
        return await ArticlePublished.findOne({
            $and: [
                { _id: articleId },
                { $or: [{ "_author": userId }, { "authorList": { $in: [userId] } }, { "editors": { $in: [userId] } }, { "reviewerList": { $in: [userId] } }] }
            ]
        }).populate(["articlePublished_data_id", "journal_info", "articleMatrices_data_id"]);
    } // end function getArticleDetail

} // end class EmployeeArticleHelper

module.exports = EmployeeArticleHelper;