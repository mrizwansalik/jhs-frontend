...
module.exports = (permission, feature) => {
    return catchAsync(async (req, res, next) => {
        ...
        if (articleInfo.status !== -1 && articleInfo.articleStatus.slug !== 'languagecorrectionservice' ) {
            // return error
            return res.status(403).json(
                Response.forbidden({ message: `Access Denied You don't have permission to revise this permission` })
            );
        }

        if (articleInfo?.assignedTo?._id == req.userId || articleInfo?.assignedTo == req.userId) {
            authorPermission = true;
            req.userType = "Assignee";
        } // end if
